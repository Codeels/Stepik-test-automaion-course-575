...
# перменные ниже даны для проверки правильности названия и цены книги/товара
...
class ProductPage(BasePage):
    ...
    def should_be_appropriate_name_in_basket(self):
        assert self.browser.find_element(*ProductPageLocators.BOOK_IN_BASKET) == self.browser.find_element(
            *ProductPageLocators.BOOK_NAME).text(), "not the same book"

    def should_be_appropriate_price_in_cart(self):
        assert self.browser.find_element(*ProductPageLocators.PRICE_IN_DESCRIPTION) == self.browser.find_element(
            *ProductPageLocators.PRICE_IN_BASKET), "not the same price"
    ...
    def should_be_correct_product_name(self):
        assert self.browser.find_element(
            *ProductPageLocators.PRODUCT_NAME).text == book_name, "product name is incorrect"
